...
function r_drawTextureComplex(path, x, y, width, height, angle) {
  //setup image
  let _img = new Image();
  ...
  ctx.save();
  ctx.translate((x-r_globalCamera.position.x)+width/2, (y-r_globalCamera.position.y)+height/2);
  ctx.rotate(angle);
  ctx.translate(-((x-r_globalCamera.position.x)+width/2), -((y-r_globalCamera.position.y)+height/2));
  
  ctx.drawImage(
    _img,
    x - r_globalCamera.position.x,
    y - r_globalCamera.position.y,
    width,
    height
  );
  ctx.restore();
